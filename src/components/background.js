...
export async function showBackground() {
  ...
  const mainColor = window.getComputedStyle(document.body).getPropertyValue('--q-primary')
  const primaryColor = window.getComputedStyle(document.body).getPropertyValue('--q-primary')
...
class BackgroundPoints {
  ...
  constructor(configs) {
    ...
    this._setStyles()
    window.addEventListener('resize', () => this._need_resize = true)
  }
  ...
  _resize() {
    this._node.width = 0
    this._node.height = 0
    this._node.width = this._node.parentNode.parentElement.clientWidth
    this._node.height = this._node.parentNode.parentElement.clientHeight
    this._scale = window.devicePixelRatio
    this._points = []
    const area = this._node.width * this._node.height

    let countPoints = Math.round(Math.sqrt(area)/5)
    if (countPoints > 1000) countPoints = 1000
    console.log(`Background resize! DPI: ${getDPI()} count points: ${countPoints}`)
  ...
  _draw() {
    if (!this._node.isConnected) return this.stop()
    if (this._need_resize) this._resize()
    this.ctx.clearRect(0, 0, this._node.width, this._node.height)
    ...
    for (let distance of this._distances) {
      ...
      if (lineWidth > this._scale) lineWidth = this._scale
